...
| Authority | Authority | — | Permission, mandate, or legitimate power governing whether a capability may be exercised within a defined scope. Authority is distinct from technical capability and from effective reachability; a subject may possess authority without a currently reachable execution path, or may obtain illicit reachability without legitimate authority. | Scoped permission to invoke a payment operation. | A stolen credential that technically works but was not legitimately granted. | Authority Graph; Capability Boundary; RFA | ADAPTED |
| Authority Graph | AuthorityGraph | — | A versioned directed graph representing subjects, identities, credentials, grants, delegation relationships, policies, services, consequential interfaces, actuators, production or resource controllers, legitimacy relationships, and revocation paths so that Legitimate Authority, Effective Reachable Authority, Illicit Reachable Authority, direct and transitive Authority Paths, and authority changes can be analyzed under declared assumptions. | Versioned IAM, service, actuator, grant, and revocation graph | A flat permission list with no delegation or reachability edges | Capability Relationship Map may contain or reference this authority-focused projection | PROPOSED |
...
| Authority Distance | AuthorityDistance | — | effort, prerequisites, and uncertainty to traverse an Authority Path | One signed grant away | Physical distance alone | Authority graph; Capability Boundary; RFA | OPEN RESEARCH |
...
| Reachable Future Authority | ReachableFutureAuthority | RFA | authority reachable through feasible actions within declared assumptions and horizon | Create role then assume it | All imaginable power | Authority graph; Capability Boundary; RFA | OPEN RESEARCH |
| Authority Expansion Gradient | AuthorityExpansionGradient | AEG | change rate and direction of reachable authority under feasible actions | Rapid delegation growth | Static authority total | Authority graph; Capability Boundary; RFA | OPEN RESEARCH |
...
| Coalition Physical Envelope | CoalitionPhysicalEnvelope | CPE | physical envelope of coordinated systems including emergent scale effects | 100-robot combined force | Single-unit rating multiplied blindly | HHR; PCE/CPE; classification | OPEN RESEARCH |
...
| Physical Sovereignty Threshold | PhysicalSovereigntyThreshold | PST | condition where critical human-controlled dependencies no longer constrain consequential autonomy | Self-sustaining energy/manufacture/repair | Owning one 3D printer | HHR; PCE/CPE; classification | OPEN RESEARCH |
| Physical Sovereignty Margin | PhysicalSovereigntyMargin | PSM | A versioned vector describing the remaining independently controlled dependencies separating a system from the Physical Sovereignty Threshold, including substitution time and cost, reserves, dependency criticality, and feasibility of external intervention. | External repair still indispensable | One universal percentage | HHR; PCE/CPE; classification | OPEN RESEARCH |
...
| Counterfactual Safety Search | CounterfactualSafetySearch | CSS | search over plausible adversarial and accidental futures for safety-relevant paths | Explore IAM escalation | Single forecast | CAG; ECA; SEP; DEV | OPEN RESEARCH |
...
| Coalition Reachable Authority | CoalitionReachableAuthority | CRA | authority reachable by coordinated actors beyond isolated authority | Shared credentials plus fleet | Simple union always | Authority graph; Capability Boundary; RFA | OPEN RESEARCH |
...
| Authority Delta | AuthorityDelta | ΔA | vector before/after change in authority graph, ceilings, paths and reach | new T3 edge | One undocumented score | Authority graph; Capability Boundary; RFA | OPEN RESEARCH |
...
| Capability Set | CapabilitySet | — | A versioned structured set of the capabilities, tools, interfaces, scopes, preconditions, operating contexts, and consequence-relevant interactions included in an assessed system configuration. It describes what the configuration is considered capable of doing and is distinct from Capability Set Version, which identifies a specific frozen Capability Set. | Versioned inventory of tools, scopes, contexts, and interactions | A version label without the frozen content | Capability Delta; Capability Drift; Authority-Changing Update; certification; lifecycle | PROPOSED |
| Capability Set Version | CapabilitySetVersion | — | An immutable identifier or content-bound version reference for one specific frozen Capability Set used by assessment, certification, change analysis, and lifecycle controls. | capset 2.1 content hash | Device model name | Capability Set; Capability Delta; Capability Drift; certification; lifecycle | PROPOSED |
| Known Hazard Catalogue | KnownHazardCatalogue | — | A versioned controlled catalogue of validated known hazards, causal patterns, affected scopes, evidence references, accepted mitigations, unresolved limitations, recurrence information, and review status used by risk, validation, incident, AIVE, knowledge, and continual-improvement processes. | Controlled catalogue linking a recurring hazard to evidence and mitigation status | A static unvalidated brainstorming list | Risk; validation; incident; AIVE; knowledge; continual improvement | ADAPTED |
| Change Safety Authority | ChangeSafetyAuthority | — | An accountable governance role authorized to approve, reject, restrict, stage, or escalate safety-relevant changes within its declared scope, while remaining distinct from the change proposer and from the controlled AI system. | Independent change board with declared scope | Automatic ultimate Legitimacy Root | Authority-Changing Update; Change Enablement; protected update path | PROPOSED |
...
| Safety Certification Expiry | SafetyCertificationExpiry | — | time or event boundary after which a certificate cannot support continued assurance without review | Certificate expires at its domain-profile-defined validity boundary or earlier after a material invalidation trigger | Permanent product badge | SAL; AHCD; certification | PROPOSED |
...
| Aggregate Effective Compute | AggregateEffectiveCompute | AEC | aggregate task-relevant computational capability across heterogeneous resources | Distributed accelerators normalized by workload | Chip count alone | UAIS-100 and applicable domain standard | OPEN RESEARCH |
...
| Human Harm Severity | HumanHarmSeverity | HHS | ordinal magnitude of credible harm independent of modality | H4 multiple-fatality or comparable widespread severe harm | Harm type | HHR; PCE/CPE; classification | PROPOSED |
...
| Evidence Supported Rating | EvidenceSupportedRating | The highest Trustworthiness Rating Level directly supported by the eligible evidence available for the declared profile scope under the applicable Trustworthiness Scoring Policy before any restrictive rating caps are applied. It is absent/null when eligible evidence cannot support `LowTrustworthiness`. | PROPOSED |
...
| Rating Explanation Record | RatingExplanationRecord | A human-readable and machine-readable explanation of why an AI Trustworthiness Rating or Trustworthiness Status changed, identifying the material evidence, confirmed errors, expired evidence, evaluator disagreement, remediation, or version change that caused the update. | PROPOSED |
...
| AI Evaluation Verdict | AIEvaluationVerdict | The structured result assigned to one evaluated scenario under an AI Evaluation Rubric, consisting of `Correct`, `AcceptableWithMinorIssue`, `MateriallyIncorrect`, `UnsafeOrCritical`, or `Indeterminate`, together with rationale, evidence references, confidence, and applicable AI Error Types. | PROPOSED |
...
| Confirmed Error Rate | ConfirmedErrorRate | The number of Confirmed AI Error Occurrences in a defined domain, task class, configuration, and evidence window divided by the corresponding valid Operational Exposure Units; reported by error type and consequence class rather than only as an aggregate. | PROPOSED |
...
| Operational Adversarial Robustness | OperationalAdversarialRobustness | The evaluated AI system's performance under domain-relevant adversarial, deceptive, ambiguous, boundary, and stress scenarios, reported by material and critical failure rates and kept distinct from the Guardian's Adversarial Reasoning Strength. | PROPOSED |
| Unresolved Material Report Exposure | UnresolvedMaterialReportExposure | The count, age, domain distribution, and exposure-normalized prevalence of AI Error Reports alleging material or critical consequences that have not yet reached a `Confirmed`, `Rejected`, `Duplicate`, or otherwise closed disposition. | PROPOSED |
...
### Machine-implementable Trustworthiness records

An `AITrustworthinessProfile` SHALL contain at least:
...
TrustworthinessScoringPolicyIdentifier
TrustworthinessScoringPolicyVersion
TrustworthinessEvidenceWindow
TrustworthinessStatus
CurrentTrustworthinessRatingLevel            # nullable
OperationalExposureSummary
TrustworthinessMetricResultsAndVersions
ConfirmedErrorSummary
UnresolvedMaterialReportSummary
IndependentEvaluationSummary
EvidenceIndependenceProfile
EvaluatorDisagreementSummary
TrustworthinessFreshnessSummary
RemediationAndRecurrenceSummary
Limitations
LastRecalculationTime
ValidityOrExpiry
HistoricalProfileReferences
SignatureOrAuditReference
```

The current rating field is nullable. `Unrated` has no active rating; `Provisional` is not `Active`; historical ratings remain visible; scope includes version, configuration, domain, task strata, and environment; and a major or material change does not automatically inherit trust.

An emitted `AITrustworthinessRating` SHALL contain at least:
...
DisagreementCap
ChangeCarryoverCap
UncertaintyAndConfidenceRecord
CalculationTimestamp
RatingExplanationRecordReference
SignatureOrAuditReference
```

If the final level is null, an active rating record is not emitted; the Profile remains present with its Status.

A `RatingChangeEvent` SHALL contain at least:

```text
RatingChangeEventIdentifier
ProfileIdentifier
PreviousTrustworthinessStatus
NewTrustworthinessStatus
PreviousTrustworthinessRatingLevel            # nullable
NewTrustworthinessRatingLevel                 # nullable
TriggeringEvidenceReferences
TrustworthinessReviewTrigger
TrustworthinessScoringPolicyIdentifier
TrustworthinessScoringPolicyVersion
AffectedDomain
...
A status-only transition and a rating-only recalculation are both valid.

### Null-safe rating derivation
...
The minimum function never receives null, and no-rating is not a fifth rating level. Status is determined independently. `Suspended`, `Withdrawn`, or `Expired` can make a computed rating non-active, and a status transition has precedence over active-rating presentation. A rating never creates Authority, never raises the Authority Ceiling, and never waives a Safety Assurance Level, Capability Boundary, physical control, or certification condition.
